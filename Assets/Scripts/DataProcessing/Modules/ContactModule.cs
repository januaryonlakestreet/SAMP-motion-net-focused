...
public class ContactModule : Module {
	...
	public static string[] ContactJoints = new string[] {"pelvis", "right_wrist", "left_wrist", "right_ankle", "left_ankle"};
	...
	public override Module Initialise(MotionData data) {
		Data = data;
		//M: This is added to automatically add the 5 contact bones of SMPLx model
		//M: Contact edit mode is not yet set. This mean contact won't be use to edit and augment the data
		AddSensor("pelvis", Vector3.zero, 0.16f, 0.0f, 0.0f, Sensor.ID.RayTopDown, Sensor.ID.None);
		AddSensor("right_wrist");
		AddSensor("left_wrist");
		AddSensor("right_ankle", Vector3.zero, 0.07f, 0.0f, 2.0f);
		AddSensor("left_ankle", Vector3.zero,0.07f,0.0f,2.0f);
		return this;
	}

	public void tmp()
    {
		Sensor right_wrist = GetSensor("right_wrist");
		right_wrist.Edit = Sensor.ID.Closest;
		right_wrist.Tolerance = 0.2f;
        right_wrist.SolveDistance = true;
        Sensor left_wrist = GetSensor("left_wrist");
		left_wrist.Edit = Sensor.ID.Closest;
		left_wrist.Tolerance = 0.2f;
        left_wrist.SolveDistance = true;

        //Sensor right = GetSensor("right_ankle");
        //right.Edit = Sensor.ID.RayTopDown;
        //Sensor left = GetSensor("left_ankle");
        //left.Edit = Sensor.ID.RayTopDown;
        //Sensor pelvis = GetSensor("pelvis");
        //pelvis.Edit = Sensor.ID.RayTopDown;
        //pelvis.Tolerance = 0.5f;
	...
	public override void Callback(MotionEditor editor) {
		if(EditMotion) {
			...
			//M: Compute where the joints should be to maintain the contact
	...
	public class Sensor {
		...
		//M: main function for data augmentation
