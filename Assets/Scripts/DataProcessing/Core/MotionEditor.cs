...
public class MotionEditor : MonoBehaviour {
	...
	//M: Added for the purpose of scheduled sampling
	private int L = 1;
		...
		if(Data != data) {
			...
			if(Data != null) {
                ...
                if (data.Modules.Length == 0)
                {
					...
					ContactModule contactModule = (ContactModule)Data.GetModule(Module.ID.Contact);
					contactModule.CaptureContactsNoCoroutine(this);
                ...
                {
					//Save = true;
					//Data.RemoveModule(Module.ID.Style);
					//Data.AddModule(Module.ID.Style);
					GoalModule goalmodule = (GoalModule)data.GetModule(Module.ID.Goal);
					goalmodule.Inspect = true;
					//contactmodule contactmodule = (contactmodule)data.getmodule(module.id.contact);
					//contactmodule.visualise = true;
					//contactModule.tmp();
					//Data.VisualiseAll(true);
				}
	...
	public void LoadFrame(float timestamp) {
		...
		if (!Callbacks)
        {
			GameObject instance = scene.GetRootGameObjects()[0];
			...
			if (instance.transform.childCount > 0)
            {
				...
				{
					instance.transform.GetChild(0).GetChild(i).gameObject.SetActive(false);
				}
				instance.transform.GetChild(0).GetChild(0).gameObject.SetActive(true);
		...
		if (frame.Index%L==0 && Callbacks)
		{
			// This part is for applying object transformations
			foreach (GameObject instance in scene.GetRootGameObjects())
			{
				//Note: this will override the local scale
				instance.transform.localScale = Vector3.one.GetMirror(Mirror ? Data.MirrorAxis : Axis.None);
				foreach (SceneEvent e in instance.GetComponentsInChildren<SceneEvent>(true))
				{
					if (Callbacks)
